--[[
	Shine commban plugin (basically the same as the ban plugin)
]]
...
function Plugin:CreateCommands()
	...
	BanCommand:Help( "<player> <duration in minutes> Bans the given player for the given time in minutes for CommBan. 0 is a permanent ban." )
	...
	local UnbanCommand = self:BindCommand( "sh_communban", "communban", Unban )
	UnbanCommand:AddParam{ Type = "string", Error = "Please specify a Steam ID to unban." }
	UnbanCommand:Help( "<steamid> Unbans the given Steam ID from CommBan." )
	...
	local function ListBans( Client )
		...
		Shine:AdminPrint( Client, "Currently stored commbans:" )
	...
	local ListBansCommand = self:BindCommand( "sh_listcommbans", nil, ListBans )
	ListBansCommand:Help( "Lists all stored commbans from Shine." )
	...
	local ForceSyncCommand = self:BindCommand( "sh_forcecommbansync", nil, ForceWebSync )
	ForceSyncCommand:Help( "Forces the commbans plugin to reload ban data from the web." )
end
...
function Plugin:CheckComLogin( Chair, Player )
	...
	if BanEntry then
		...
		if not BanEntry.UnbanTime or BanEntry.UnbanTime == 0 or BanEntry.UnbanTime > Time() then --Either a perma-ban or not expired.
			Shine:Notify( Player, "[CommBans]", "", "You were banned from being a commander. Reason: %s", true, BanEntry.Reason )
			return false
		else
			self:RemoveBan( ID )
		end
...
function Plugin:ReceiveRequestBanData( Client, Data )
	if not Shine:GetPermission( Client, "sh_communban" ) then return end
