...
Shine:RegisterExtension( "commban", Plugin )
...
function Plugin:SetupAdminMenu()
	...
	local function OpenAddBanWindow()
		...
		Window:AddTitleBar( "Add CommBan" )
		Window:SkinColour()
		...
		function AddBan.DoClick()
			...
			Shine.AdminMenu:RunCommand( "sh_commbanid", StringFormat( "%s %s %s", ID, Duration, Reason ) )
	...
	self:AddAdminMenuTab( "CommBans", {
		OnInit = function( Panel, Data )
			...
			function Unban.DoClick()
				...
				Shine.AdminMenu:RunCommand( "sh_communban", ID )
			end
